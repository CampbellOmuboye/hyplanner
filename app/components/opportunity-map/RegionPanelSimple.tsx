...
function ReadinessBar({ value }: { value: number }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="w-9 shrink-0 text-right text-sm font-medium text-neutral-700">{value}</span>
    </div>
  );
}
            ...
            <>
              <div className="flex items-start justify-between border-b border-neutral-100 bg-neutral-50/80 p-5">
                <div className="min-w-0 flex-1 pr-3">
                  <h2 className="text-xl font-semibold tracking-tight text-neutral-900">{region.name}</h2>
                  <span
                    className="mt-2 inline-block rounded-full px-3 py-1 text-xs font-medium text-white shadow-sm"
                    style={{
                      backgroundColor:
                        OPPORTUNITY_COLORS[region.classification as OpportunityClassification],
                    }}
                  >
              ...
              <div className="flex-1 overflow-y-auto p-5">
                <section className="mb-6">
                  ...
                  <p className="text-sm leading-relaxed text-neutral-700">{region.summary}</p>
                </section>
